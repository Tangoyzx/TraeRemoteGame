...
- The player always starts with an independent basic attack: one 50-damage bullet every 2 seconds, aimed at the nearest enemy.
- The upgrade system is currently reset for a redesign. Upgrade thresholds and popup UI remain in code, but options are empty and triggering is disabled, so the level display stays at `Level 0`.

## Skills and upgrades

The default bullet attack is a permanent combat system rather than an upgradeable skill choice. Legacy weapon, stat, elemental, and debuff implementations have been removed. The current reset baseline is documented in `docs/skill-system-framework.md`.

## Project structure

```text
scenes/main.tscn        Main Godot scene
scripts/main.gd         Game loop, map, spawning, UI, and disabled upgrade shell
scripts/basic_attack.gd Permanent automatic player attack
scripts/player.gd       Player movement, HP, damage handling
scripts/enemy.gd        Enemy config, movement, HP, score value
scripts/projectile.gd   Basic-attack projectile behavior
docs/                   Current skill-system reset baseline
